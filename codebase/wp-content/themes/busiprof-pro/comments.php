...
add_filter('comment_form_default_fields','my_fields'); 
	...
	'comment_field'        => '<div class="form-group col-xs-12"><textarea rows="5" id="comment" name="comment" type="text" placeholder="'. __("Message",'busiprof' ).'" rows="3"></textarea></div>',		
	 'logged_in_as' => '<div class="col-xs-12"><p class="logged-in-as">' . __("Logged in as ",'busiprof' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="'.__("Logout of this account",'busiprof').'">'.' '.__("Logout",'busiprof').'</a>' . '</p></div>',
